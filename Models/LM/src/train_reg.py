...
from tensorflow.python.keras.losses import BinaryCrossentropy, CategoricalCrossentropy, MeanSquaredError
...
def classifier(X_train, X_dev, Y_train, Y_dev, config, model_name):
    ...
    # print(f"DEBUG {config['model'].upper()!r}")
...
def main():
    ...
    X_train, Y_train, X_dev, Y_dev = load_data(DATA_DIR, config)
